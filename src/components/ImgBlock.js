...
class ImgBlock extends Component {
  constructor(props) {
    ...
    this.refreshAnnotationInterval = this.refreshAnnotationInterval.bind(this);
    this.refreshImageState = this.refreshImageState.bind(this);
    this.postAnnotation = this.postAnnotation.bind(this);
    this.postAnnotationAndGetNext = this.postAnnotationAndGetNext.bind(this)
  ...
  panLeft() {
    ...
    if (Number(canvasCoords['x'] - 0.1) > 0) {
      var currentCenter = this.coordImageToCanvas(this.state.x_center, this.state.y_center);
      var newXCenter = currentCenter['x'] + 0.1;
      var newCenter = this.coordCanvasToImage(newXCenter, this.state.y_center);
  ...
  panUp() {
    ...
    if (Number(canvasCoords['y'] - 0.1) > 0) {
      ...
      var newCenter = this.coordCanvasToImage(this.state.x_center, newYCenter);
  ...
  panDown() {
    ...
    console.log("coords x, y: " + canvasCoords['x'] + ", " + canvasCoords['y']);
  ...
  getMainImageContainer() {
    return document.getElementById(this.props.prefix + "MainImageContainer")
  }
  ...
  coordCanvasToImage(x, y, doLog = false) {
    ...
    var deltaXCanvas = (x - .5);
    var deltaYCanvas = (y - .5);
    ...
    if (doLog) {
      console.log("x, y: " + x + ", " + y);
      console.log("deltaXCanvas, deltaYCanvas: " + deltaXCanvas + ", " + deltaYCanvas);
      console.log("deltaXImg, deltaYImg: " + deltaXImg + ", " + deltaYImg);
      console.log("-----coordCanvasToImage() End-----");
    }
    return { 'x': this.state.x_center + deltaXImg, 'y': this.state.y_center + deltaYImg };
  }
  ...
  coordImageToCanvas(x, y, doLog = false) {
    ...
    if (doLog) {
      console.log("x, y: " + x + ", " + y);
      console.log("deltaXImage, deltaYImage: " + deltaXImage + ", " + deltaYImage);
      console.log("deltaXCanvas, deltaYCanvas: " + deltaXCanvas + ", " + deltaYCanvas);
      console.log("-----coordImageToCanvas() End-----");
    }

    return { 'x': .5 + deltaXCanvas, 'y': .5 + deltaYCanvas };
  }
  ...
  handleImageErrored() {
    ...
    var centeredX = .5 * canvas.width
    var centeredY = .5 * canvas.height
    ...
    context.fillText(bottomErrorMessage, centeredX, centeredY + context.measureText('M').width);
  }
  ...
  convertMS(milliseconds) {
    ...
    hour = hour % 24;
    return (day.toString().padStart(2, "0") + " Days, " + hour.toString().padStart(2, "0") + ":" + minute.toString().padStart(2, "0") + ":" + seconds.toString().padStart(2, "0"));
  }
  ...
  drawTimer(doLog = false) {
    ...
    } while ((context.measureText(Date.now()).width >= (canvasWidth * textwidthScale)) | (context.measureText(Date.now()).height >= canvasHeight))
    ...
    context.fillText(this.convertMS(this.timeUntilExpiration), centeredX, centeredY);
  }
  ...
  async getImageToAnnotate(table) {
    var geturi = vizserverURI + "/get_image_to_annotate?annotation_table=" + table;
    log("geturi", geturi)
  ...
  async postRefreshAnnotationInterval(imageId, table) {
    var posturi = vizserverURI + "/refresh_annotation_interval?image_id=" + imageId + "&annotation_table=" + table;
  ...
  async postAnnotation() {
    ...
    var posturi = vizserverURI + "/insert_annotation?image_id=" + this.state.imgId + "&annotation_table=" + this.props.annotation_table;
  ...
  getNextImage() {
    ...
    var result = this.getImageToAnnotate(
  ...
  postAnnotationAndGetNext() {
    this.postAnnotation().then(res => {
      ...
      if (error.response.status == 422) {
        throw Error("Bad post submission for annotation: ", error.response)
      }
    })
  }
  ...
  refreshAnnotationInterval() {
    var result = this.postRefreshAnnotationInterval(this.state.imgId, this.props.annotation_table).then(imageToAnnotate => {
      this.refreshImageState(imageToAnnotate)
    }).catch(error => {
      if (error.response.status == 422) {
        throw Error("Bad post submission for annotation interval refresh: ", error.response)
      }
    })
  }
  ...
  removeEventListeners() {
    ...
    this.getMainImageContainer().removeEventListener("mouseover", this.handleMouseEnter, false);
    this.getMainImageContainer().removeEventListener("mouseout", this.handleMouseLeave, false);
  }

  componentDidMount() {
    document.addEventListener("keydown", this.handleKeyPress, false);
    document.addEventListener("wheel", this.handleWheel, false);
    this.getMainImageContainer().addEventListener("mouseover", this.handleMouseEnter, false);
    this.getMainImageContainer().addEventListener("mouseout", this.handleMouseLeave, false);
  ...
  componentDidUpdate(prevProps) {
    if (this.props.annotation_table != prevProps.annotation_table) {
      ...
      // Use this to use promises for loading images
      var result = this.getImageToAnnotate(this.props.annotation_table).then(imageToAnnotate => {
        this.setNewImageState(imageToAnnotate)
      }).catch(error => {
        this.setBlankImageState()
