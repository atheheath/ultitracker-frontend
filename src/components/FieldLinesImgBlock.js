...
class FieldLinesImgBlock extends ImgBlock {
  constructor(props) {
    ...
    this.defaultState = Object.assign({}, super.defaultState, defaultStateExtension)

    this.colors = ["#a0e3b7", "#9d0d6c", "#37c454", "#eb67f9", "#769d31", "#330045"];
  ...
  handleMouseDown(e, doLog = true) {
    ...
    if (this.state.mouseIsOver) {
      ...
      var mouseLocImage = this.coordCanvasToImage(mouseLocCanvas['x'], mouseLocCanvas['y']);
      ...
      for (var segmentIndex in this.state.lineSegments) {
        ...
        var endpoints = this.getEndPoints(lseg['x1'], lseg['y1'], lseg['x2'], lseg['y2']);
      ...
      if (isOverRectangle) {
        ...
        for (var segmentIndex in this.state.lineSegments) {
          if (segmentIndex != lsegIndex) {
            newLineSegments[segmentIndex] = this.state.lineSegments[segmentIndex];
          }
        }
        ...
        var imageCoords = this.coordCanvasToImage(canvasCoords['x'], canvasCoords['y']);
        this.setState({
          "tempLineSegment": {
            'x1': imageCoords['x'],
            'y1': imageCoords['y'],
            'x2': imageCoords['x'] + (.1 / this.state.scale),
            'y2': imageCoords['y'] + (.1 / this.state.scale),
            'anchor': 1
          }
        });
      }
  ...
  handleMouseUp(e, doLog = true) {
    ...
    if (doLog) {
      ...
      console.log("newLineSegment x1, y1, x2, y2: " + newLineSegment['x1'] + ", " + newLineSegment['y1'] + ", " + newLineSegment['x2'] + ", " + newLineSegment['y2']);
    }
  ...
  handleMouseMove(e, doLog = false) {
    ...
    if (!(this.state.tempLineSegment === null)) {
      ...
      if (doLog) {
        console.log("canvasCoords x, y: " + canvasCoords['x'] + ", " + canvasCoords['y']);
        console.log("imageCoords x, y: " + imageCoords['x'] + ", " + imageCoords['y']);
      }
      var newLineSegment = {
        'x1': tempLineSegment['x1'],
        'y1': tempLineSegment['y1'],
  ...
  getEndPoints(x1, y1, x2, y2, doLog = false) {
    ...
    var squareDimCanvasY = squareDimCanvasX * (canvas.width / canvas.height);
    ...
    if (doLog) {
      console.log("squareDimCanvasX, Y: " + squareDimCanvasX + ", " + squareDimCanvasY);
      console.log("squareDimImageX, Y: " + squareDimImageX + ", " + squareDimImageY);
    }
    ...
    if (doLog) {
      console.log("tlX, Y, W, H: " + point1X + ", " + point1Y + ", " + point1W + ", " + point1H);
      console.log("brX, Y, W, H: " + point2X + ", " + point2Y + ", " + point2W + ", " + point2H);
  ...
  drawLineSegment(x1, y1, x2, y2, { color = "#000000", lineWidth = 1, doLog = false }) {
    var canvas = this.getCanvas();
    ...
    if (doLog) {
      console.log("lseg_x1, lseg_y1, lseg_x2, lseg_y2: " + canvasX1 + ", " + canvasY1 + ", " + canvasX2 + ", " + canvasY2);
    }
  ...
  drawRect(x, y, w, h, { color = "#000000", lineWidth = 1, doLog = false }) {
    ...
    if (doLog) {
      console.log("rect_x, rect_y, rect_w, rect_h: " + canvasX + ", " + canvasY + ", " + canvasWidth + ", " + canvasHeight);
    }
  ...
  drawText(x, y, w, h, text) {
    ...
    } while ((context.measureText(text).width >= canvasWidth) | (context.measureText(text).height >= canvasHeight))

    context.fillStyle = "greenyellow";
    context.textBaseline = "middle";
    context.textAlign = "center";
    context.fillText(text, canvasX + canvasWidth / 2, canvasY + canvasHeight / 2);
  }

  // draws little squares around the top left and bottom right corners of a
  // rectangle to be able to modify existing rectangles
  drawEndPoints(x1, y1, x2, y2, { color = "#000000", lineWidth = 1, doLog = false }) {
    if (doLog) {
    ...
    var point1 = endpoints['point1'];
    var point2 = endpoints['point2'];
    this.drawRect(point1['x'], point1['y'], point1['w'], point1['h'], { color: color, lineWidth: lineWidth, doLog: doLog });
    this.drawRect(point2['x'], point2['y'], point2['w'], point2['h'], { color: color, lineWidth: lineWidth, doLog: doLog });

    this.drawText(point1['x'], point1['y'], point1['w'], point1['h'], "1");
    this.drawText(point2['x'], point2['y'], point2['w'], point2['h'], "2");

    if (doLog) {
      log("tlX, Y, W, H: " + point1["x"], point1["y"], point1["w"], point1["h"]);
      log("brX, Y, W, H: " + point2["x"], point2["y"], point2["w"], point2["h"]);
  ...
  drawLegend() {
    ...
    for (var i = 0; i < fieldLinesNames.length; i++) {
      ...
      do {
        fontsize--;
        context.font = fontsize + "px Helvetica";
      } while ((context.measureText(temptext).width >= (canvasWidth * textwidthScale)) | (context.measureText(temptext).height >= step))
    }
  ...
  drawImage(doLog = false) {
    ...
    for (var segmentIndex in this.state.lineSegments) {
      ...
      if (segmentIndex == this.state.activeSegment) {
        this.drawLineSegment(lseg['x1'], lseg['y1'], lseg['x2'], lseg['y2'], { color: this.colors[segmentIndex], lineWidth: 10, doLog: doLog });
        this.drawEndPoints(lseg['x1'], lseg['y1'], lseg['x2'], lseg['y2'], { color: this.colors[segmentIndex], lineWidth: 10, doLog: doLog });
      } else {
        this.drawLineSegment(lseg['x1'], lseg['y1'], lseg['x2'], lseg['y2'], { color: this.colors[segmentIndex], doLog: doLog });
        this.drawEndPoints(lseg['x1'], lseg['y1'], lseg['x2'], lseg['y2'], { color: this.colors[segmentIndex], doLog: doLog });
      }
    ...
    if (!(this.state.tempLineSegment === null)) {
      ...
      this.drawLineSegment(lseg['x1'], lseg['y1'], lseg['x2'], lseg['y2'], { color: this.colors[this.state.activeSegment], lineWidth: 10, doLog: doLog });
    }
  ...
  removeEventListeners() {
    ...
    this.getCanvas().removeEventListener("mousedown", this.handleMouseDown, false);
    this.getCanvas().removeEventListener("mouseup", this.handleMouseUp, false);
    this.getCanvas().removeEventListener("mousemove", this.handleMouseMove, false);
  }

  componentDidMount() {
    super.componentDidMount();
    this.getCanvas().addEventListener("click", this.handleClick, false);
    this.getCanvas().addEventListener("mousedown", this.handleMouseDown, false);
    this.getCanvas().addEventListener("mouseup", this.handleMouseUp, false);
    this.getCanvas().addEventListener("mousemove", this.handleMouseMove, false);
  }
