...
class PlayerBboxImgBlock extends ImgBlock {
  ...
  handleMouseDown(e, doLog = false) {
    ...
    if (this.state.mouseIsOver) {
      ...
      for (var i = 0; i < this.state.rects.length; i++) {
        ...
        var endpoints = this.getEndPoints(rect['x'], rect['y'], rect['w'], rect['h']);
        var tl = endpoints['tl'];
        var br = endpoints['br'];
      ...
      if (isOverRectangle) {
        ...
        var newRects = this.state.rects.slice(0, rectIndex).concat(this.state.rects.slice(rectIndex + 1, this.state.rects.length));
        console.log("rects, newRects Length: " + this.state.rects.length + ", " + newRects.length);

        // if we clicked the top left corner, then the bottom right corner
        // now the anchor, and vice versa
        if (isTl) {
          rectToAdjust['initialX'] = rectToAdjust['x'] + rectToAdjust['w'];
          rectToAdjust['initialY'] = rectToAdjust['y'] + rectToAdjust['h'];
        }
        else {
        ...
        this.setState({
          'tempRect': {
            'x': imageCoords['x'],
            'y': imageCoords['y'],
            'w': (.1 / this.state.scale),
            'h': (.1 / this.state.scale),
  ...
  handleMouseUp(e, doLog = false) {
    ...
    if (doLog) {
      console.log("tempRectInitialX,Y: " + tempRect['initialX'] + ", " + tempRect['initialY']);
      console.log("newRect x, y, w, h: " + newRect['x'] + ", " + newRect['y'] + ", " + newRect['w'] + ", " + newRect['h']);
    }
  ...
  getEndPoints(x, y, w, h, doLog = false) {
    ...
    if (doLog) {
      console.log("tlX, Y, W, H: " + tlX + ", " + tlY + ", " + tlW + ", " + tlH);
      console.log("brX, Y, W, H: " + brX + ", " + brY + ", " + brW + ", " + brH);
  ...
  drawEndPoints(x, y, w, h, { color = "#000000", lineWidth = 1, doLog = false }) {
    if (doLog) {
    ...
    var tl = endpoints['tl'];
    var br = endpoints['br'];
    this.drawRect(tl['x'], tl['y'], tl['w'], tl['h'], { color: color, lineWidth: lineWidth, doLog: doLog });
    this.drawRect(br['x'], br['y'], br['w'], br['h'], { color: color, lineWidth: lineWidth, doLog: doLog });

    if (doLog) {
      console.log("tlX, Y, W, H: " + tl["x"] + ", " + tl["y"] + ", " + tl["w"] + ", " + tl["h"]);
      console.log("brX, Y, W, H: " + br["x"] + ", " + br["y"] + ", " + br["w"] + ", " + br["h"]);
  ...
  drawImage(doLog = false) {
    ...
    for (var i = 0; i < this.state.rects.length; i++) {
      ...
      if (i == this.state.activeRect) {
        this.drawRect(rect['x'], rect['y'], rect['w'], rect['h'], { lineWidth: 10, doLog: doLog });
        this.drawEndPoints(rect['x'], rect['y'], rect['w'], rect['h'], { lineWidth: 10, doLog: doLog });
      } else {
        this.drawRect(rect['x'], rect['y'], rect['w'], rect['h'], { doLog: doLog });
        this.drawEndPoints(rect['x'], rect['y'], rect['w'], rect['h'], { doLog: doLog });
      }
